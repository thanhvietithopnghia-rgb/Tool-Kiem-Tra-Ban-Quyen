...
$tempRoot = Join-Path ([IO.Path]::GetTempPath()) ("Tool-Kiem-Tra-v4.3-extensions-" + [Guid]::NewGuid().ToString("N"))
...
'@, (New-Object Text.UTF8Encoding($false)))
    ...
    $timelineState = Initialize-ToolLicenseTimeline -ToolVersion "4.3"
    ...
    $envelope = New-ToolReportEnvelope -ReportKind "CertificateAudit" -ToolVersion "4.3" -Data ([ordered]@{
    ...
    try {
        ...
        $validation = Test-ToolReportEnvelope -Report $json -ExpectedReportKind "CertificateAudit" -ExpectedToolVersion "4.3"
    ...
    try {
        ...
        if ($tempFull.StartsWith($expectedPrefix, [StringComparison]::OrdinalIgnoreCase) -and
            [IO.Path]::GetFileName($tempFull).StartsWith("Tool-Kiem-Tra-v4.3-extensions-", [StringComparison]::OrdinalIgnoreCase) -and
