...
- **August 11 hotfix — secure UAC bridge:** fixes `SecureLaunchRequired` when Windows RunAs does not preserve the Dashboard environment block. Every administrative module now uses an encoded bootstrap that transfers only the allowlisted `TOOL_*` contract variables, validates schema/age/module/invocation, restores the protected runtime, and returns the child exit code to the Dashboard. Secure-launch, ACL, and selection-file checks remain fail-closed instead of being disabled as a workaround.
